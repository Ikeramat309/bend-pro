...
Honest snapshot of where the app stands. **Phases 1, 2, and 3 are complete.** The app is now in **Phase 4** (more calculators — none started yet). Keep this doc updated when the picture changes — and never describe planned work as if it exists.
...
Both follow the same feature pattern: `*.config.ts`, `*.copy.ts`, `engine/` (pure math + types), `ui/` (screen + diagram). Both show a **bender profile context** banner under setup explaining where values come from (chart, override, or default fallback). Both produce warnings for invalid or impractical inputs and format results via shared `formatLength`.
...
- **More calculators** — kick, saddles, rolling offset, segment bending, etc. Not started. See [`ROADMAP.md`](ROADMAP.md) and [`PRODUCT_BRIEF.md`](PRODUCT_BRIEF.md).
