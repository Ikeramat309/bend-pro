...
const DBB_VECTOR_OFFSET = 36;
const HEIGHT_VECTOR_X = 326;
const HEIGHT_LABEL_X = 310;
...
function OffsetDistanceVector({ geo, value }: { geo: OffsetDiagramGeometry; value: string }) {
  ...
  const vectorOffset = DBB_VECTOR_OFFSET + (1 - geo.sinA) * 8;
  const start = {
    x: geo.x1 - normal.x * vectorOffset,
    y: MODEL.bottomY - normal.y * vectorOffset,
  };
  const end = {
    x: geo.x2 - normal.x * vectorOffset,
    y: geo.topY - normal.y * vectorOffset,
  };
  const mid = midpoint(start, end);
  const labelTarget = { x: 121, y: 50 };
...
function OffsetHeightVector({ geo, value }: { geo: OffsetDiagramGeometry; value: string }) {
  ...
  const midY = (MODEL.bottomY + geo.topY) / 2;
  const titleY = compact ? geo.topY - 44 : midY - 8;
  const valueY = titleY + 16;
  const labelTarget = { x: HEIGHT_LABEL_X + 5, y: titleY + 8 };
...
function OffsetLiveDiagram({ data }: { data: OffsetDiagramData }) {
  ...
  const tag2 = {
    x:
      geo.rise < 42
        ? Math.max(mark2Width / 2 + 4, mark2.x - 42)
        : Math.min(MODEL.endX - mark2Width / 2 - 5, mark2.x + 32),
    y: geo.topY - (geo.rise < 42 ? 21 : 26),
  };
